...
\usepackage[light,scaled=0.8]{roboto-mono}
...
\usetheme[$for(themeoptions)$$themeoptions$$sep$,$endfor$block=fill,progressbar=frametitle]{metropolis}
...
\setmainlanguage[$polyglossia-lang.options$]{$lang$}
...
$endif$
$if(biblatex)$
\usepackage[$if(biblio-style)$style=$biblio-style$,$endif$$for(biblatexoptions)$$biblatexoptions$$sep$,$endfor$,backend=biber,hyperref=true]{biblatex}
$for(bibliography)$
...
% \newcommand{\theHtable}{\thetable} conflicto al compilar
...
\begin{document}
	...
	\begin{frame}[plain,noframenumbering]
		...
		$if(social)$
		$if(social.correo)$\faEnvelopeO \texttt{$social.correo$}$endif$	$if(social.twitter)$\hspace{.3cm} \faTwitter \texttt{\href{https://twitter.com/$social.twitter$}{$social.twitter$}}$endif$ $if(social.github)$\hspace{.3cm} \faGithub \texttt{\href{https://github.com/$social.github$}{$social.github$}}$endif$
