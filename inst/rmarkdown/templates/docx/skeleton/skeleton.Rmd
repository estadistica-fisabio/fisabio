...
```{r portada_plus, echo = FALSE, results='asis'}
###########################################################
## NO BORRAR ESTA SECCIÓN SI SE QUIERE PORTADA           ##
###########################################################
mk_line <- function(x) officer::fpar(officer::ftext(x, ft), fp_p = fp_params)

bl <- officer::block_list(
  mk_line(params$autores),
  mk_line(sprintf("Versión %s (%s)", params$version, Sys.Date())),
  mk_line(params$instituciones),
  mk_line(params$email)
)

officedown::knit_print_block(bl)
```
